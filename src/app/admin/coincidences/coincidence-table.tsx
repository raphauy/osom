...
export function DataTableToolbar<TData>({
  ...
  return (
    <div className="flex gap-1 dark:text-white">
      ...
      <Input
        className="max-w-xs"
        placeholder="pedidoId filter..."
        value={(table.getColumn("pedidoId")?.getFilterValue() as string) ?? ""}
        onChange={(event) =>
          table.getColumn("pedidoId")?.setFilterValue(event.target.value)
        }
      />

      <Input
        className="max-w-xs"
        placeholder="propertyId filter..."
        value={
          (table.getColumn("propertyId")?.getFilterValue() as string) ?? ""
        }
        onChange={(event) =>
          table.getColumn("propertyId")?.setFilterValue(event.target.value)
        }
      />
