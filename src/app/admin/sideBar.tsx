...
import { Briefcase, ChevronRightSquare, FlaskConical, LayoutDashboard, MessageCircle, PackageOpen, Settings, User } from "lucide-react";
...
export default function SideBar() {
  ...
  const selectedClasses= "font-bold text-intraprop-color dark:text-white"
  ...
  const pedidosSelected= path.endsWith("pedidos")
  const pedidos= clsx(commonClasses, pedidosSelected  && selectedClasses)
  ...
  return (
    <div className="flex flex-col justify-between border-r border-r-intraprop-color/50">
      <section className="flex flex-col gap-3 py-4 mt-3 ">
        ...
        <Link href="/admin/pedidos" className={pedidos}>
          <PackageOpen />
          <p className={pClasses}>Pedidos</p>                  
        </Link>
...
function divider() {
  return <div className="mx-2 my-5 border-b border-b-intraprop-color/50 dark:border-b-white/50" />
}
