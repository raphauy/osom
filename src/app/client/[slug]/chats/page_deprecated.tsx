...
import { getDataConversationAction, getLastDataConversationAction } from "./actions"
...
export default async function ChatPage({ searchParams: { id }, params: { slug } }: Props) {

  let conversation;
  if (!id) {
    if (!slug) return <div>Cliente no encontrado</div>

    conversation= await getLastDataConversationAction(slug)
  } else {
    conversation= await getDataConversationAction(id)
  }
